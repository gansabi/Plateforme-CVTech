...
import { inscrireCandidat } from '../services'
...
export function InscriptionCandidat() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setChargement(true)
    try {
      await inscrireCandidat({ email, motDePasse })
      setSucces(true)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-green-100 p-6 rounded-lg text-center">
          <p className="text-green-800 font-medium">Compte créé ! Redirection vers la connexion...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="max-w-md w-full bg-white p-8 rounded-lg shadow">
        <h1 className="text-2xl font-bold text-center mb-6">Inscription Candidat</h1>
